<template>
    <div class="col-md-12" id="axiosForm">
        ...
        <div class="box box-primary">
            <form role="form" @submit.prevent="status ? updateProfile() : createProfile()" enctype="multipart/form-data" method="POST" action="/profile" >
                <fieldset :disabled="loading">
                    <div class="box-body">
                        <div class="form-row">
                            <div class="form-group col-md-5">
                                <label for="name">Name</label>
                                <input type="text" class="form-control" v-model="profile.name" id="name" placeholder="Company/Organization/Individual name" :disabled="toggleStatus" required>
                                <span class="help-block text-danger" v-if="errors && errors.name">{{errors.name[0]}}</span>
                            </div>

                            <div class="form-group col-md-4">
                                <label>Email</label>
                                    <input type="email" class="form-control" v-model="profile.email"  placeholder="example@example.com" :disabled="toggleStatus" required>
                                    <span class="help-block text-danger" v-if="errors && errors.email">{{errors.email[0]}}</span>
                            </div>

                            <div class="form-group col-md-3">
                                <label>Phone Number</label>
                                <input type="text" class="form-control" v-model="profile.phone" placeholder="Phone Number" :disabled="toggleStatus" required>
                            ...
                            <div class="form-group col-md-12">
                                ...
                                <textarea class="form-control" rows="3" placeholder="Address" v-model="profile.address" style="resize:none" :disabled="toggleStatus" required></textarea>
                        ...
                        <div class="form-row">
                            <div class="form-group col-md-12">
                                <div class="color-palette-set">
                                    <div class="bg-green disabled color-palette p-1"><span>Vehicle Information</span></div>
                                </div>
                            </div>
                            <div class="form-group col-md-6">
                                <label>Registration No.</label>
                                <input type="text" class="form-control" placeholder="Reg. No." v-model="profile.registration_no" :disabled="toggleStatus" required>
                                <span class="help-block text-danger" v-if="errors && errors.registration_no">{{errors.registration_no[0]}}</span>
                            </div>
                            <div class="form-group col-md-6">
                                <label>Chasis No.</label>
                                <input type="text"  class="form-control" placeholder="Chasis No." v-model="profile.chasis_no" :disabled="toggleStatus" required>
                                <span class="help-block text-danger" v-if="errors && errors.chasis_no">{{errors.chasis_no[0]}}</span>
                            </div>
                            <div class="form-group col-md-8">
                                <label><strong>Driver's License</strong></label>
                                <input type="file" @change="uploadLicense" class="form-control" placeholder="Chasis No." :disabled="toggleStatus">
                                <!-- <vue-dropify :multiple='dropify.multiple' :message="dropify.message"></vue-dropify> -->
                                <span class="help-block text-danger" v-if="errors && errors.driving_license">{{errors.driving_license[0]}}</span>
                            </div>
                            <div class="form-group col-md-4">
                                <label><strong>License Expiration</strong></label>
                                <input type="date" v-model="profile.license_expiration" class="form-control" placeholder="Expirationn Date"  :min="min_date" :disabled="toggleStatus" required>
                                <!-- <vue-dropify :multiple='dropify.multiple' :message="dropify.message"></vue-dropify> -->
                                <span class="help-block text-danger" v-if="errors && errors.license_expiration">{{errors.license_expiration[0]}}</span>
                            </div>
                        </div>
                        <div class="form-row" v-for="(fields, index) in profile.other_fields" :key="fields.id">
                            ...
                            <div class="form-group col-md-4">
                                <input type="file" @change="uploadDoc(index)" class="form-control" placeholder="Chasis No." :disabled="toggleStatus">
                            </div>
                            <div class="form-group col-md-3">
                                <input type="date" v-model="fields.expiry_date" class="form-control" placeholder="Expirationn Date"  :min="min_date" :disabled="toggleStatus" required>
                            </div>

                            <span v-show="!toggleStatus" @click="removeField(index)" class="text-danger text-center col-md-1"><i class="fas fa-minus-circle" ></i></span>

                        </div>

                        <div class="form-row" v-show="!toggleStatus">
                            <label id="add_doc" @click="addField" ><span class="text-success"><i class="fas fa-plus"></i> Add other documents</span></label>
                        </div>
                        <div class="form-row">
                            <div class="form-group col-md-12">
                                <label for="location">Other info</label>
                                <textarea class="form-control" rows="3" placeholder="Other Information" v-model="profile.other_info" style="resize:none" :disabled="toggleStatus"></textarea>
        ...
        validateDuplicate(fields, index) {
            ...
            if(this.profile.other_fields.length > 1) {
                ...
                if(current_field.length > 1) {
                    swal.fire(
                        'Error',
                        'You already Selected ' + fields.title,
                        'error'
                    )
                    this.removeField(index)
                }
